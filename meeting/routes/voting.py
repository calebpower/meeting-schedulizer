...
def vote(request):
    ...
    project_list = pull_projects(profile)
    for i in range(len(project_list)):
        for j in range(len(project_list[i])):
            print(project_list[i][j].project_name)
            m = models.Meeting.objects.filter(project=project_list[i][j])
            for meeting in m:
...
def availability_save_handler(sender, instance, **kwargs):
    ...
    meeting_time_option = MeetingTimeOption.objects.filter(
    ...
    if instance.meeting.state == REVIEW:
        print('State: ' + str(instance.meeting.state))
        Meeting.objects.filter(id=instance.meeting.id).update(
                    ...
                    if FATS[k].start_time == availableTimeSlots[j].start_time and FATS[k].end_time == availableTimeSlots[j].end_time:
                        ...
                        if set(a) == set(b):
                            ...
                            if set(a) - set(b) and not set(b) - set(a):
                                ...
                                print(
                                    'set(set(b) - set(a)) != set(set(b) - set(a))')
                            else:
    ...
    def generate_meeting_time_option(self, start, end):
        ...
        if abs(end - start) >= self.MEETING_DURATION and abs(end - start) < self.MEETING_DURATION*2:
            ...
            while True:
                ...
                temp = TempTimeSlot(self.milli_to_datetime(
                    rStart), self.milli_to_datetime(rStart+self.MEETING_DURATION))
        ...
        while u < len(teamMember):
            ...
            for s in range(len(teamMember[u].timeSlots)):
                if not self.checkMember(teamMember[u].timeSlots[s].members, teamMember[u].name):
                    teamMember[u].timeSlots[s].members.append(
                        teamMember[u].name)
...
# ****************************Initilize values************************************
        ...
        project_list = pull_projects(profile)
        print('project_list length:' + str(len(project_list)))
        print('project_list:' + str(project_list))
        time_slots = models.TimeAvailability.objects.filter(
            user=profile, meeting_id=meeting_id)
        json_data = models.TimeAvailability.objects.all()
        time_slots_json = "["
        for datum in json_data:
            meeting = '"meeting":{"id":"' + str(datum.meeting.id) + \
                '","description":"' + datum.meeting.description + '"}'
            time_slots_json += '{"id":"' + str(datum.id) + '","start_time":"' + str(
                datum.start_time) + '","end_time":"' + str(datum.end_time) + '",' + meeting + '},'
        if len(time_slots_json) > 1:
            time_slots_json = time_slots_json[:-1]
        time_slots_json += ']'

        project = [
            meeting for meeting in meeting_list if meeting.id == meeting_id][0].project
        members = models.Member.objects.filter(project=project)
        meeting = models.Meeting.objects.get(
            id=meeting_id) if models.Meeting.objects.get(id=meeting_id) else None
        ops = models.MeetingTimeOption.objects.filter(meeting_id=meeting_id)
        print('meeting_time_options length is ' +
              str(len(meeting_time_options)))
        for op in ops:
        ...
        for member in members:
            ...
            if timeslots:
                ...
                for slot in timeslots:
                    ...
                    tTest = TempTimeSlot(
                        slot.start_time, slot.end_time)  # For Debug
                ...
                if meeting.state == NO_VOTE or meeting.state == DONE:
                    ...
                    Meeting.objects.filter(id=meeting_id, project=project).update(
                        state=models.Meeting.VoteState.CLOSE)
        ...
        try:
            ...
            if not meeting_time_options and state_ready:
                ...
                print('g_asl done')
                print(len(g_asl))
                r_slots = self.refine_meeting_time_slots(
                    self.refine_meeting_time_slots_lst(g_asl))
                print('r_slots done')
                print(len(r_slots))
                meeting_time_slots = self.find_meeting_slots(r_slots, team)
                print('meeing_time_slots len = ' +
                      str(len(meeting_time_slots)))
                ...
                if len(meeting_time_options) == 0:
                    ...
                    Meeting.objects.filter(id=meeting_id, project=project).update(
                        state=models.Meeting.VoteState.NO_VOTE)
    ...
    # *******************************Check vote state***************************************
        ...
        if meeting.state == DONE:
            ...
            if len(votes) == len(members):
                ...
                meeting_time_options.sort(
                    key=lambda MeetingTimeOption: MeetingTimeOption.vote_count, reverse=True)
                if meeting_time_options[0].vote_count == meeting_time_options[1].vote_count:
                    ...
                    for m in range(len(meeting_time_options)):
                        if m != 0 and m != 1:
                            if meeting_time_options[0].vote_count == meeting_time_options[m].vote_count:
                                print(m)
                                fifty_fifty_votes.append(
                                    meeting_time_options[m])
                    ...
                    ffv = []
                    for v in votes:
                        ffv.append(MeetingTimeOption(start_time=v.meeting_time_option.start_time,
                                                     end_time=v.meeting_time_option.end_time, meeting=meeting))
    ...
    def post(self, request, meeting_id, *args, **kwargs):
        ...
        meeting = models.Meeting.objects.get(
            id=meeting_id) if models.Meeting.objects.get(id=meeting_id) else None
        meeting_time_option_id = int(request.POST.get(
            'id')) if request.POST.get('id') else None
        ...
        meeting_time_option = models.MeetingTimeOption.objects.get(
            id=meeting_time_option_id, meeting=meeting) if models.MeetingTimeOption.objects.get(id=meeting_time_option_id, meeting=meeting) else None
        if not models.Vote.objects.filter(user=profile, meeting_id=meeting_id):
            vote = Vote(meeting_time_option=meeting_time_option,
        ...
        if request.POST.get('delete') if request.POST.get('delete') else None:
            print('Vote Deleted')
            Vote.objects.filter(
                meeting_time_option=meeting_time_option, meeting=meeting, user=profile).delete()
...
def voting(request):
    ...
    for meeting in meeting_list:
        avlb_count = models.TimeAvailability.objects.filter(
            meeting_id=meeting.id).count()
        avlb_meeting_list.append(
            {'meeting': meeting, 'avlb_count': avlb_count})
